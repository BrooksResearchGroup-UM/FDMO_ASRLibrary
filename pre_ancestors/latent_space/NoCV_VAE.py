...
# layers = [100]
# Size of batchs for batch processing
...
dim = 2
# You need to change "new_folder" as an output folder for each training.dim = 
'''CHANGE THIS!!!'''
new_folder = '20230920_3_PF01494'
# 19 was screened to be the best seed based on the reconstruction accuracy of the testing dataset
seed_list = [19]

# Current directory (you might need to change this):
'''CHANGE THIS!!!'''
os.chdir("/home/cdchiang/for_azamh/vae")
current_dir = os.getcwd()

# Make a new folder for the training
os.mkdir("./{}".format(new_folder))

# Load dataset:¶

input_file = 'data/PF01494_proc_output_5_20230920_2'
...
for seed in seed_list:
    ...
    # testing dataset (ancestral sequences)
    ...
    .format(current_dir, input_file, num_seq, len_protein, len_protein*num_res_type, len(layers), layers, dim, 'Adam', weight_decay, KLD_beta, num_epoches, batch_size_train, lr, DEVICE, seed))
    print(' ')
    ...
    vae.cpu()
    torch.save(vae.state_dict(), "./{}/NoCV_vae_d{}_lr{}_w{}_b{}_l{}_beta{}_{}epoch_seed{}.model".format(new_folder, dim, len(layers), str(weight_decay), batch_size_train, lr, KLD_beta, num_epoches, seed))
    ...
    ## start plotting the latent space

    # load the model
    vae.cuda()
    vae.load_state_dict(torch.load("./{}/NoCV_vae_d{}_lr{}_w{}_b{}_l{}_beta{}_{}epoch_seed{}.model".format(new_folder, dim, len(layers), str(weight_decay), batch_size_train, lr, KLD_beta, num_epoches, seed)))
    ...
    with open("./{}/NoCV_reconstruction_d{}_layer{}_w{}_b{}_l{}_beta{}_{}epoch_seed{}.pkl".format(new_folder, dim, len(layers), str(weight_decay), batch_size_train, lr, KLD_beta, num_epoches, seed), 'wb') as file_handle:
    ...
    with open("./{}/NoCV_reconstruction_d{}_layer{}_w{}_b{}_l{}_beta{}_{}epoch_seed{}.pkl".format(new_folder, dim, len(layers), str(weight_decay), batch_size_train, lr, KLD_beta, num_epoches, seed), 'rb') as file_handle:
    ...
    # start plotting the reconstruction curves.
    plt.figure(0)
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig("./{}/NoCV_reconstruction_d{}_layer{}_w{}_b{}_l{}_beta{}_{}epoch_seed{}.png".format(new_folder, dim, len(layers), str(weight_decay), batch_size_train, lr, KLD_beta, num_epoches, seed))

    # save key, mu, and sigma (This is important!)
    # you can get the latent space representations of all sequences by loading this pkl file.
    with open("./{}/NoCV_latent_space_d{}_layer{}_w{}_b{}_l{}_beta{}_{}epoch_seed{}.pkl".format(new_folder, dim, len(layers), str(weight_decay), batch_size_train, lr, KLD_beta, num_epoches, seed), 'wb') as file_handle:
        pickle.dump({'key': key_list, 'mu': mu, 'sigma': sigma, 't_key': t_key_list, 't_mu': t_mu, 't_sigma': t_sigma}, file_handle) 

    # load the latent space info (This is important!)
    with open("./{}/NoCV_latent_space_d{}_layer{}_w{}_b{}_l{}_beta{}_{}epoch_seed{}.pkl".format(new_folder, dim, len(layers), str(weight_decay), batch_size_train, lr, KLD_beta, num_epoches, seed), 'rb') as file_handle:
    ...
    plt.ylabel("$Z_2$")
    plt.tight_layout()
    plt.savefig("./{}/NoCV_latent_mu_scatter_d{}_layer{}_w{}_b{}_l{}_beta{}_{}epoch_seed{}.png".format(new_folder, dim, len(layers), str(weight_decay), batch_size_train, lr, KLD_beta, num_epoches, seed))
